...
const mockAmounts = computed(() => ({
    ...
    rate: new Intl.NumberFormat(locale.value === 'fa' ? 'fa-IR' : locale.value, {
        style: 'percent',
    }).format(0.87),
}));
...
const howSteps = [
    ...
    { key: 'insights', icon: BarChart3, color: '#F59E0B', bg: 'bg-[#F59E0B]/10' },
] as const;

const faqKeys = ['private', 'calendar', 'telegram', 'free', 'currencies'] as const;
...
<template>
    <Head :title="t('landing.meta.title')">
        ...
        <meta property="og:title" :content="`CashPilot — ${t('landing.meta.title')}`" />
        <meta property="og:description" :content="t('landing.meta.description')" />
        <meta property="og:image" content="/apple-touch-icon.png" />
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" :content="`CashPilot — ${t('landing.meta.title')}`" />
    ...
    <div class="min-h-screen overflow-x-hidden bg-[#0a0a0a] text-white">
        ...
        >
            ...
            >
                <!-- Logo -->
                <Link :href="home()" class="flex cursor-pointer items-center gap-2.5">
                ...
                <div class="hidden items-center gap-3 lg:flex">
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <ChevronDown class="size-3.5 transition-transform" :class="{ 'rotate-180': langMenuOpen }" />
                        </button>
                        <div
                            v-if="langMenuOpen"
                        ...
                        >
                            ...
                            >
                                ...
                                <Check v-if="code === locale" class="size-4 text-[#02CD86]" />
                            </button>
                        </div>
                    </div>
                ...
                <button
                    ...
                    :aria-label="mobileMenuOpen ? t('landing.a11y.close_menu') : t('landing.a11y.open_menu')"
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-2 border-t border-white/10 pt-3">
                        ...
                        <button
                            ...
                            :class="code === locale ? 'bg-[#02CD86]/15 text-[#02CD86]' : 'text-white/60 hover:text-white'"
                            @click="switchLocale(code)"
                        >
                            {{ localeLabels[code] ?? code }}
                        </button>
        ...
        >
            ...
            <div class="relative z-10 mx-auto max-w-5xl text-center">
                ...
                <!-- Headline -->
                <h1 class="mb-6 text-5xl leading-tight font-bold tracking-tight text-white sm:text-6xl lg:text-7xl rtl:tracking-normal">
                ...
                >
                    ...
                    <div class="mb-5 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs text-white/50">{{ t('landing.hero.mock.dashboard') }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <div class="rounded-md bg-[#2d2d2d] px-3 py-1 text-[10px] text-white/50">{{ t('landing.hero.mock.currency') }}</div>
                    ...
                    <div class="mb-4 grid grid-cols-3 gap-3">
                        <div class="rounded-[14px] bg-[#1a1a1a] p-3">
                            <div class="mb-2 flex items-center gap-1.5">
                                ...
                                <span class="text-[9px] font-medium tracking-wider text-white/40 uppercase rtl:tracking-normal">{{ t('landing.hero.mock.income') }}</span>
                            </div>
                            <div class="text-sm font-bold text-white">{{ mockAmounts.income }}</div>
                            <div class="text-[9px] text-white/30">{{ t('landing.hero.mock.toman') }}</div>
                        </div>
                        <div class="rounded-[14px] bg-[#1a1a1a] p-3">
                            <div class="mb-2 flex items-center gap-1.5">
                                ...
                                <span class="text-[9px] font-medium tracking-wider text-white/40 uppercase rtl:tracking-normal">{{ t('landing.hero.mock.costs') }}</span>
                            </div>
                            <div class="text-sm font-bold text-white">{{ mockAmounts.costs }}</div>
                            <div class="text-[9px] text-white/30">{{ t('landing.hero.mock.toman') }}</div>
                        </div>
                        <div class="rounded-[14px] bg-[#1a1a1a] p-3">
                            <div class="mb-2 flex items-center gap-1.5">
                                ...
                                <span class="text-[9px] font-medium tracking-wider text-white/40 uppercase rtl:tracking-normal">{{ t('landing.hero.mock.balance') }}</span>
                            </div>
                            <div class="text-sm font-bold text-[#02CD86]" dir="ltr">{{ mockAmounts.balance }}</div>
                            <div class="text-[9px] text-white/30">{{ t('landing.hero.mock.toman') }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-[1fr_2fr] gap-3">
                        ...
                        <div class="flex flex-col items-center justify-center rounded-[14px] bg-[#1a1a1a] px-3 py-4">
                            <div class="text-[9px] font-medium tracking-wider text-white/40 uppercase rtl:tracking-normal">{{ t('landing.hero.mock.finance_rate') }}</div>
                            <div class="my-2 flex h-14 w-14 items-center justify-center rounded-full border-4 border-[#02CD86] bg-[#0a0a0a]">
                                <span class="text-sm font-bold text-[#02CD86]">{{ mockAmounts.rate }}</span>
                            </div>
                            <div class="text-[8px] text-white/30">{{ t('landing.hero.mock.rate_hint') }}</div>
                        </div>
                        <!-- Bar chart mock -->
                        <div class="rounded-[14px] bg-[#1a1a1a] p-3">
                            <div class="mb-2 flex items-center justify-between">
                                <span class="text-[9px] text-white/40">{{ t('landing.hero.mock.monthly_overview') }}</span>
                                <div class="flex items-center gap-2">
                                    <span class="flex items-center gap-1 text-[8px] text-white/30"><span class="inline-block h-1.5 w-1.5 rounded-full bg-[#02CD86]" />{{ t('landing.hero.mock.income') }}</span>
                                    <span class="flex items-center gap-1 text-[8px] text-white/30"><span class="inline-block h-1.5 w-1.5 rounded-full bg-[#6C4EE9]" />{{ t('landing.hero.mock.costs') }}</span>
                                </div>
                            </div>
        ...
        <!-- ══════════════════════════════════════════════════════
             TRUST STRIP
        ═════════════════════════════════════════════════════════ -->
        <section class="border-y border-white/[0.06] bg-white/[0.02] px-4 py-10" :aria-label="t('landing.a11y.trust')">
            <div class="mx-auto grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                >
                    ...
                    <div>
                        <div class="text-sm font-bold text-white">{{ t(`landing.trust.${item.key}.title`) }}</div>
                        <div class="mt-0.5 text-sm text-white/40">{{ t(`landing.trust.${item.key}.text`) }}</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <!-- ══════════════════════════════════════════════════════
             FEATURES BENTO GRID
        ═════════════════════════════════════════════════════════ -->
        <section id="features" class="scroll-mt-24 px-4 py-24" aria-labelledby="features-heading">
            <div class="mx-auto max-w-6xl">
                <!-- Section header -->
                <div class="mb-16 text-center">
                    <p class="mb-3 text-sm font-semibold tracking-[0.3em] text-[#02CD86] uppercase rtl:tracking-normal">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    >
                        ...
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.transactions.title') }}</h3>
                        <p class="mb-5 leading-relaxed text-white/50">
                            {{ t('landing.features.transactions.text') }}
                        </p>
                        <ul class="flex flex-col gap-2">
                            <li v-for="i in 3" :key="i" class="flex items-center gap-2 text-sm text-white/60">
                                <Check class="size-4 shrink-0 text-[#02CD86]" />
                                {{ t(`landing.features.transactions.points.${i - 1}`) }}
                            </li>
                        </ul>
                    </div>
                    ...
                    >
                        ...
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.investments.title') }}</h3>
                        <p class="mb-5 leading-relaxed text-white/50">
                            {{ t('landing.features.investments.text') }}
                        </p>
                        <!-- Asset chips -->
                        <div class="mb-5 flex flex-wrap gap-2">
                            <span v-for="asset in ['🥇', '🥈', '💵', '💶', '₿']" :key="asset" class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/60">
                                {{ asset }}
                            </span>
                        </div>
                        <ul class="flex flex-col gap-2">
                            <li v-for="i in 3" :key="i" class="flex items-center gap-2 text-sm text-white/60">
                                ...
                                {{ t(`landing.features.investments.points.${i - 1}`) }}
                            </li>
                        </ul>
                    </div>
                    ...
                    >
                        ...
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.bills.title') }}</h3>
                        <p class="mb-5 leading-relaxed text-white/50">
                            {{ t('landing.features.bills.text') }}
                        </p>
                        <ul class="flex flex-col gap-2">
                            <li v-for="i in 2" :key="i" class="flex items-center gap-2 text-sm text-white/60">
                    ...
                    >
                        ...
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.reports.title') }}</h3>
                    ...
                    >
                        <div class="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#3B82F6]/10">
                            <DollarSign class="size-6 text-[#3B82F6]" />
                        </div>
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.multi_currency.title') }}</h3>
                        <p class="leading-relaxed text-white/50">
                    ...
                    >
                        <div class="pointer-events-none absolute top-0 right-0 h-40 w-40 rounded-full bg-[#02CD86]/[0.08] blur-2xl" />
                        <div class="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#02CD86]/10">
                            <ShieldCheck class="size-6 text-[#02CD86]" />
                        </div>
                        <h3 class="mb-2 text-xl font-bold text-white">{{ t('landing.features.security.title') }}</h3>
        ...
        <!-- ══════════════════════════════════════════════════════
             TELEGRAM SPOTLIGHT
        ═════════════════════════════════════════════════════════ -->
        <section id="telegram" class="scroll-mt-24 border-y border-white/[0.06] bg-white/[0.02] px-4 py-24" aria-labelledby="telegram-heading">
            <div class="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-2">
                <!-- Copy side -->
                <div>
                    <p class="mb-3 text-sm font-semibold tracking-[0.3em] text-[#3B82F6] uppercase rtl:tracking-normal">
                        {{ t('landing.telegram.kicker') }}
                    </p>
                    <h2 id="telegram-heading" class="text-4xl font-bold text-white sm:text-5xl">
                    ...
                    <div class="mt-10 flex flex-col gap-6">
                        <div v-for="point in telegramPoints" :key="point.key" class="flex items-start gap-4">
                            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#3B82F6]/10">
                                <component :is="point.icon" class="size-5 text-[#3B82F6]" />
                            </div>
                            <div>
                                <h3 class="font-bold text-white">{{ t(`landing.telegram.points.${point.key}.title`) }}</h3>
                                <p class="mt-1 text-sm text-white/50">{{ t(`landing.telegram.points.${point.key}.text`) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Phone chat mockup -->
                <div class="mx-auto w-full max-w-sm" aria-hidden="true" role="presentation">
                    <div class="rounded-[2.5rem] border border-white/10 bg-[#141414] p-3 shadow-[0_40px_80px_rgba(0,0,0,0.6)]">
                        <div class="overflow-hidden rounded-[2rem] bg-[#0e1621]">
                            <!-- Chat header -->
                            <div class="flex items-center gap-3 border-b border-white/5 bg-[#17212b] px-4 py-3">
                                <div class="flex h-9 w-9 items-center justify-center rounded-full bg-[#02CD86]">
                                    <img :src="logoGreen" alt="" class="h-5 w-4" />
                                </div>
                                <div>
                                    <div class="text-sm font-semibold text-white">{{ t('landing.telegram.chat.bot_name') }}</div>
                                    <div class="text-[11px] text-[#02CD86]">{{ t('landing.telegram.chat.status') }}</div>
                                </div>
                            ...
                            <div class="flex flex-col gap-2.5 px-3 py-4">
                                <!-- Bot: reminder -->
                                <div class="max-w-[85%] self-start rounded-2xl rounded-bl-md bg-[#182533] px-3.5 py-2.5 text-[13px] leading-relaxed text-white/90">
                                    {{ t('landing.telegram.chat.reminder') }}
                                    <div class="mt-2 rounded-lg border border-[#02CD86]/40 bg-[#02CD86]/10 px-3 py-1.5 text-center text-[12px] font-semibold text-[#02CD86]">
                                ...
                                <!-- User: add expense -->
                                <div class="max-w-[75%] self-end rounded-2xl rounded-br-md bg-[#2b5278] px-3.5 py-2.5 text-[13px] text-white">
                                    {{ t('landing.telegram.chat.user_add') }}
                                </div>
                                <!-- Bot: wizard -->
                                <div class="max-w-[85%] self-start rounded-2xl rounded-bl-md bg-[#182533] px-3.5 py-2.5 text-[13px] text-white/90">
        ...
        <!-- ══════════════════════════════════════════════════════
             HOW IT WORKS
        ═════════════════════════════════════════════════════════ -->
        <section id="how-it-works" class="scroll-mt-24 px-4 py-24" aria-labelledby="hiw-heading">
            <div class="mx-auto max-w-4xl">
                <div class="mb-16 text-center">
                    <p class="mb-3 text-sm font-semibold tracking-[0.3em] text-[#6C4EE9] uppercase rtl:tracking-normal">
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    ...
                    >
                        ...
                        <h3 class="mb-2 text-lg font-bold text-white">{{ t(`landing.how.steps.${step.key}.title`) }}</h3>
                        <p class="text-sm leading-relaxed text-white/50">{{ t(`landing.how.steps.${step.key}.text`) }}</p>
        ...
        <!-- ══════════════════════════════════════════════════════
             FAQ
        ═════════════════════════════════════════════════════════ -->
        <section id="faq" class="scroll-mt-24 px-4 py-24" aria-labelledby="faq-heading">
            <div class="mx-auto max-w-3xl">
                <div class="mb-12 text-center">
                    <p class="mb-3 text-sm font-semibold tracking-[0.3em] text-[#02CD86] uppercase rtl:tracking-normal">
                        {{ t('landing.faq.kicker') }}
                    </p>
                    <h2 id="faq-heading" class="text-4xl font-bold text-white sm:text-5xl">
                        {{ t('landing.faq.title') }}
                    </h2>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    >
                        <button
                            ...
                            @click="toggleFaq(key)"
                        >
                            <span class="font-semibold text-white">{{ t(`landing.faq.items.${key}.q`) }}</span>
                            <ChevronDown
                        ...
                        <div v-if="openFaq === key" class="px-6 pb-5">
                            <p class="leading-relaxed text-white/50">{{ t(`landing.faq.items.${key}.a`) }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        ═════════════════════════════════════════════════════════ -->
        <section class="relative overflow-hidden px-4 py-24" :aria-label="t('landing.a11y.call_to_action')">
            ...
            <div class="relative z-10 mx-auto max-w-3xl text-center">
                <!-- Compact free banner -->
                <div class="mb-10 inline-flex flex-col items-center gap-1 rounded-2xl border border-[#02CD86]/30 bg-[#02CD86]/[0.06] px-8 py-4 sm:flex-row sm:gap-3">
                    <span class="font-bold text-[#02CD86]">{{ t('landing.pricing.title') }}</span>
                    <span class="hidden text-white/20 sm:inline">—</span>
                    <span class="text-sm text-white/50">{{ t('landing.pricing.text') }}</span>
                </div>
        ...
        <footer class="border-t border-white/[0.06] px-4 py-10" role="contentinfo">
            <div class="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 sm:flex-row">
                ...
                <p class="text-sm text-white/30">
                    © {{ new Date().getFullYear() }} CashPilot. {{ t('landing.footer.tagline') }}
                </p>
