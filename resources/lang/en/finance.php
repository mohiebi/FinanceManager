<?php

return [
    'actions' => [
        'add_cost' => 'Add Cost',
        'add_income' => 'Add Income',
        'add_entry' => 'Add Entry',
        'add_first_entry' => 'Add first entry',
        'apply_custom_range' => 'Apply custom range',
        'filter' => 'Filter',
        'export_transactions' => 'Export',
        'import_transactions' => 'Import',
        'reset_filters' => 'Reset filters',
        'see_all' => 'See all ->',
        'sync_prices' => 'Sync prices',
        'bulk_delete' => 'Delete selected',
        'bulk_assign' => 'Assign category',
    ],
    'assets' => [
        'gold' => 'Gold',
        'silver' => 'Silver',
        'usd' => 'US Dollar',
        'eur' => 'Euro',
        'coin' => 'Bahar Azadi',
        'bitcoin' => 'Bitcoin',
        'total' => 'Total Portfolio',
    ],
    'asset_classes' => [
        'metal' => 'Precious metal',
        'currency' => 'Currency',
        'crypto' => 'Crypto',
        'stock' => 'Stock',
        'etf' => 'ETF',
        'bond' => 'Bond',
        'commodity' => 'Commodity',
        'real_estate' => 'Real estate',
        'private_asset' => 'Private asset',
        'other' => 'Other',
        'unclassified' => 'Unclassified',
    ],
    'bills' => [
        'title' => 'Bills',
        'add_bill' => 'Add bill',
        'edit_bill' => 'Edit bill',
        'description' => 'Set up recurring or one-time bills and we will remind you a day before and on the due date.',
        'recurrence' => 'Recurrence',
        'recurrence_monthly' => 'Monthly',
        'recurrence_one_time' => 'One-time',
        'due_day_of_month' => 'Day of month',
        'due_day_label' => 'Day {day}',
        'due_date' => 'Due date',
        'payment_duration' => 'Payment duration',
        'continues_indefinitely' => 'Continues indefinitely',
        'limit_by_count' => 'Set number of payments',
        'limit_by_date' => 'End on a specific date',
        'total_payments' => 'Total payments',
        'last_payment_date' => 'Last payment date',
        'count_hint' => 'Payments will be numbered from 1 to {count}.',
        'date_hint' => 'Choose the last date and we will calculate the number of payments.',
        'calculated_payments' => 'This schedule contains {count} payments in total.',
        'payment_progress' => 'Payment {current} of {total}',
        'payments_complete' => 'All {total} payments complete',
        'telegram_reminder' => 'Remind me on Telegram',
        'telegram_reminders_title' => 'Telegram reminders',
        'telegram_reminders_description' => 'The bot messages you before each bill lands. Turn reminders on for a bill from its edit dialog.',
        'telegram_reminders_empty' => 'No bills have a Telegram reminder on yet.',
        'reminder_time' => 'Reminder time',
        'reminder_timezone' => 'Timezone',
        'next_due' => 'Next due',
        'upcoming' => 'Upcoming Bills',
        'no_upcoming' => 'No upcoming due date',
        'due_today' => 'Due today',
        'due_in_days' => 'Due in {days} days',
        'overdue' => 'Overdue',
        'scheduled' => 'Scheduled',
        'mark_paid' => 'Mark as paid',
        'paid' => 'Paid',
        'paid_on' => 'Paid {date}',
        'empty' => 'No bills yet',
        'empty_description' => 'Add a bill like rent or phone payment and we will remind you before it is due.',
        'invalid_category' => 'Please choose a valid category.',
        'limit_before_paid' => 'The total cannot be less than the {count} payments already made.',
        'end_before_next' => 'Choose a date that includes at least the next payment.',
        'limit_too_large' => 'A payment plan can contain at most {count} payments.',
        'list_view' => 'List',
        'calendar_view' => 'Calendar',
        'due_soon_title' => 'Due in the next {days} days',
        'balance_covers' => 'Your balance covers all of them, with {amount} to spare.',
        'balance_short' => "Your balance is short by {amount} for what's due.",
        'header_subtitle_none_overdue' => '{count} scheduled · none overdue',
        'header_subtitle_overdue' => '{count} scheduled · {overdue} overdue',
    ],
    'calculating' => 'Calculating…',
    'price_unavailable' => 'Price unavailable',
    'last_synced' => 'Updated {time}',
    'sync_too_soon' => '{1}You can sync again in 1 minute.|[2,*]You can sync again in :minutes minutes.',
    'calendar' => [
        'current_period' => 'Current Period',
        'day_of_month' => 'Day {day} of {days}',
        'elapsed' => '{progress}% of month elapsed',
        'days_left' => '{days} days left',
    ],
    'currencies' => [
        'toman' => 'TOMAN',
        'usd' => 'USD',
        'eur' => 'EURO',
    ],
    'categories' => [
        'cost' => [
            'food' => 'Food',
            'transport' => 'Transport',
            'housing' => 'Housing',
            'health' => 'Health',
            'shopping' => 'Shopping',
            'bills' => 'Bills',
            'other' => 'Other',
        ],
        'income' => [
            'salary' => 'Salary',
            'freelance' => 'Freelance',
            'gift' => 'Gift',
            'investment' => 'Investment',
            'other' => 'Other',
        ],
        'add_custom' => 'New category',
        'already_exists' => 'That category already exists for this type.',
        'parent_invalid' => 'Choose one of your categories or a default category as the parent.',
        'parent_is_self' => 'A category cannot be its own parent.',
        'parent_not_top_level' => 'Subcategories cannot have their own subcategories. Pick a top-level parent.',
        'has_children' => 'This category has subcategories, so it cannot become one itself.',
        'parent_type_mismatch' => 'The parent has to be available for the same types as this category.',
        'children_need_shared' => 'Some of its subcategories are used for the other type. Change them first.',
        'unshare_in_use' => ':count transaction(s) of the other type still use this category. Move them to another category first.',
        'name_placeholder' => 'Category name',
        'new_name' => 'New category name',
        'uncategorized' => 'Uncategorized',
    ],
    'dashboard' => [
        'title' => 'Dashboard',
        'subtitle' => '{month} {year} · {days}',
        'eyebrow' => 'Finance command center',
        'heading' => 'Your money, on Autopilot.',
        'welcome' => 'Welcome back,',
        'guest' => 'there',
        'monthly_overview' => 'Monthly overview',
        'spending_by_category' => 'Spending by Category',
        'total_spent' => 'Total spent',
        'daily_spending' => 'Daily Spending',
        'cost_optimize' => 'Cost Optimize',
        'savings_rate' => 'Savings rate',
        'recent_costs' => 'Recently Costs',
        'recent_incomes' => 'Recently Incomes',
        'no_costs' => 'No costs yet. Add one when money leaves.',
        'no_incomes' => 'No incomes yet. Add income when money comes in.',
        'live_prices' => 'Live Prices',
        'top_holdings' => 'Top holdings',
        'assets_count' => '{count} asset | {count} assets',
        'portfolio_empty' => 'No investments yet',
        'portfolio_empty_cta' => 'Add your first entry',
        'safe_to_spend' => 'Safe to spend today',
        'safe_to_spend_explanation' => 'You have spent {cost} of {income} so far in {month}, with {days} days left. At this pace, that leaves about {amount} a day.',
        'safe_to_spend_no_days' => 'The period is closing — check your balance before spending more.',
        'needs_decision' => 'Needs a decision',
        'needs_decision_body' => 'You have spent {amount} more than you brought in this {month}. Review your recent transactions to see what to adjust.',
        'review_transactions' => 'Review transactions',
        'needs_you_this_week' => 'Needs you this week',
        'where_it_went' => 'Where it went',
        'cash_flow' => 'Cash flow this month',
        'cash_flow_hint' => 'Hover a day to see the exact figures',
        'net_worth' => 'Net worth',
        'per_day' => 'per day',
    ],
    'delete' => [
        'transaction_title' => 'Delete "{title}"?',
        'transaction_description' => 'This transaction will be permanently deleted.',
        'bulk_transactions_title' => 'Delete {count} selected transactions?',
        'bulk_transactions_description' => 'These transactions will be permanently deleted.',
        'investment_title' => 'Delete investment entry?',
        'investment_description' => 'This entry will be permanently removed from your portfolio.',
        'bill_title' => 'Delete "{title}"?',
        'bill_description' => 'This bill and its reminders will be permanently deleted.',
    ],
    'fields' => [
        'amount' => 'Amount',
        'asset' => 'Asset',
        'category' => 'Category',
        'currency' => 'Currency',
        'date' => 'Date',
        'description' => 'Description',
        'display_currency' => 'Display currency',
        'from' => 'From',
        'holdings' => 'Holdings',
        'note' => 'Note',
        'optional' => 'optional',
        'quantity' => 'Quantity',
        'quantity_short' => 'Qty',
        'range' => 'Range',
        'search' => 'Search',
        'subject' => 'Subject',
        'to' => 'To',
        'type' => 'Type',
        'total_cost' => 'Total cost',
        'value' => 'Value',
        'cost_basis_per_unit' => 'Cost basis / unit',
        'avg_cost_per_unit' => 'Avg cost / unit',
    ],
    'filters' => [
        'all_categories' => 'All categories',
        'uncategorised' => 'Uncategorised only',
        'exclude_investments' => 'Exclude investments',
        'exclude_investments_hint' => 'Money moved into assets is not counted as spending',
        'all_types' => 'All types',
        'costs' => 'Costs',
        'incomes' => 'Incomes',
        'select_asset' => 'Select asset',
        'select_currency' => 'Select currency',
        'title_or_note' => 'Title or note',
        'rows_per_page' => 'Rows per page',
        'per_page_option' => '{count} per page',
    ],
    'form' => [
        'add_cost' => 'Add cost',
        'add_income' => 'Add income',
        'edit_cost' => 'Edit cost',
        'edit_income' => 'Edit income',
        'add_investment' => 'Add investment entry',
        'edit_investment' => 'Edit entry',
        'transaction_description' => 'The same form handles both tables. The transaction type follows the table action you selected.',
        'investment_description' => 'Record how much of an asset you held on a given date.',
        'subject_placeholder' => 'Hamburger, Fresh Restaurant',
        'note_placeholder' => 'Optional note',
        'investment_note_placeholder' => 'e.g. bought at Tejarat bank',
    ],
    'import' => [
        'title' => 'Import transactions',
        'action' => 'Import',
        'footer_hint' => 'Import a CSV of transactions and review every row before saving it.',
        'description' => 'Upload a CSV with English headers. Values may be English or Persian, including Jalali dates, Persian digits, and Rial amounts.',
        'prompt_title' => 'AI conversion prompt',
        'copy_prompt' => 'Copy prompt',
        'copied' => 'Copied',
        'download_template' => 'Download CSV template',
        'file_label' => 'CSV file',
        'preview' => 'Preview',
        'confirm' => 'Import valid rows',
        'total_rows' => 'Total rows',
        'importable_rows' => 'Importable',
        'invalid_rows' => 'Invalid',
        'duplicate_rows' => 'Duplicates',
        'row' => 'Row',
        'status' => 'Status',
        'notes' => 'Notes',
        'ready' => 'Ready',
        'result' => 'Imported {imported} transaction(s). Skipped {skipped}: {duplicates} duplicate(s), {invalid} invalid row(s).',
        'failed' => 'The request could not be completed. Please try again.',
        'expired' => 'This preview is unavailable or expired. Please upload the CSV again.',
        'statuses' => [
            'valid' => 'Valid',
            'invalid' => 'Invalid',
            'duplicate' => 'Duplicate',
        ],
    ],
    'investments' => [
        'title' => 'Investments',
        'sell' => 'Sell Entity',
        'kind_buy' => 'Buy',
        'kind_sell' => 'Sell',
        'entries_grouped_by_asset' => '{count} records, grouped by asset',
        'nothing_to_sell' => 'You have nothing to sell yet.',
        'sell_asset' => 'Sell {asset}',
        'sell_description' => 'A sale is recorded on its own, leaving the purchase it came from untouched.',
        'sell_quantity' => 'How much are you selling?',
        'sell_total' => 'Total you sold it for',
        'sell_exceeds_holding' => 'You cannot sell more than you currently hold.',
        'sell_not_editable' => 'A sale cannot be edited. Delete it and record it again.',
        'update_leaves_negative_holding' => 'This change would leave you holding less than nothing. Adjust or remove the sales of this asset first.',
        'holding_now' => 'You hold {quantity} {unit}',
        'record_cost' => 'Also record this as a cost',
        'record_cost_hint' => 'Adds a transaction under Investment for the money that left your pocket',
        'record_income' => 'Also record the proceeds as income',
        'record_income_hint' => 'Adds the full amount you received, not just the profit',
        'bought_title' => 'Bought {quantity} {unit} {asset}',
        'sold_title' => 'Sold {quantity} {unit} {asset}',
        'realised' => 'Realised',
        'realised_hint' => 'Profit you have already banked by selling',
        'unrealised' => 'Unrealised',
        'total_portfolio_value' => 'Total Portfolio Value',
        'asset_types' => 'Asset Types',
        'held' => 'held',
        'yours' => 'Yours',
        'total_entries' => 'Total Entries',
        'records' => 'records',
        'investment_entries' => 'Investment entries',
        'market_prices_kicker' => 'Live prices',
        'market_prices_title' => 'Asset prices right now',
        'market_prices_unit' => 'Per 1 unit in {currency}',
        'market_prices_common' => 'Market assets',
        'market_prices_custom' => 'Your custom assets',
        'per_unit' => '1 {unit}',
        'no_entries_title' => 'No investments yet',
        'no_entries_description' => 'Start tracking your assets. Add your first entry to see your portfolio come to life.',
    ],
    'metrics' => [
        'balance' => 'Balance',
        'costs' => 'Costs',
        'costs_excluding_investments' => 'Costs excl. investments',
        'income' => 'Income',
        'in_the_positive' => 'In the positive',
        'overspent' => 'Overspent',
        'transactions' => 'transaction | transactions',
        'entries' => 'entry | entries',
    ],
    'portfolio' => [
        'title' => 'Portfolio',
        'overview' => 'Portfolio overview',
        'allocation' => 'Allocation',
        'by_current_value' => 'by current value',
        'value_over_time' => 'Value over time',
        'add_a_series' => 'Add a series',
        'hero_title' => 'Your holdings, cost basis & P/L at a glance.',
        'hero_description' => 'Add a cost basis when logging investment entries to unlock profit/loss tracking here.',
        'export_profit_loss' => 'Export P&L',
        'export_profit_loss_hint' => 'Download your portfolio profit and loss report as a spreadsheet.',
        'current_value' => 'Current value',
        'cost_basis' => 'Cost basis',
        'total_cost_basis' => 'Total cost basis',
        'profit_loss' => 'Profit / Loss',
        'return' => 'Return',
        'pnl_by_asset' => 'Profit / Loss by asset',
        'invested' => 'Invested',
        'no_cost_basis' => 'No cost basis yet',
        'per_asset_breakdown' => 'Per-asset breakdown',
        'entry_history' => 'Entry history',
        'entry_level_detail' => 'Holdings detail',
        'entry_history_description' => 'Merged holdings by asset with quantity, average cost, total cost, current value, and P/L.',
        'empty' => 'Portfolio is empty',
        'empty_description' => 'Head over to the Investments page and add your first entry. Come back here to track your profit & loss once you add cost basis info.',
        'net_worth' => 'Net worth',
        'net_worth_description' => 'The total value of everything you hold, right now.',
        'assets_held' => 'Assets held',
        'today' => 'Today',
    ],
    'reports' => [
        'title' => 'Reports',
        'eyebrow' => 'Time-based reports',
        'heading' => 'Slice your money story by month, season, year, or any custom range.',
        'description' => 'Each filter refreshes the transaction list and totals so you can compare what came in and what went out over the period you care about.',
        'transactions' => 'Transactions',
        'this_month' => 'This month',
        'this_season' => 'This season',
        'yearly' => 'Yearly',
        'custom' => 'Custom',
        'cash_flow' => 'Cash Flow',
        'top_spending' => 'Top Spending Categories',
        'net_savings' => 'Net Savings',
        'by_category' => 'by category',
        'break_down' => 'Break down:',
        'by_day' => 'by day',
        'by_month' => 'by month',
        'read_eyebrow' => 'The read',
        'read_headline_positive' => 'You came out {amount} ahead',
        'read_headline_negative' => 'You spent {amount} more than you brought in',
        'read_body_spending' => '{category} was the biggest slice of your spending, at {percent}% of the period\'s costs.',
        'read_body_transfers' => '{category} was the biggest slice, at {percent}% — with investments counted as transfers rather than spending.',
        'read_body_no_spending' => 'No costs were logged in this period yet.',
        'export_hint' => 'Download your complete transaction ledger as a spreadsheet.',
    ],
    'tables' => [
        'money_coming_in' => 'Money coming in',
        'money_going_out' => 'Money going out',
    ],
    'transactions' => [
        'title' => 'Transactions',
        'subtitle' => '{count} entries this period',
        'select_row' => 'Select transaction',
        'select_all' => 'Select all on this page',
        'selected_count' => '{count} selected',
        'bulk_type_mixed' => 'Select one table at a time to reassign a category.',
    ],
];
