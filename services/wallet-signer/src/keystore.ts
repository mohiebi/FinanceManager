...
export async function derivePublicAddresses(
    path: string,
    start: number,
    count: number,
): Promise<{
    ...
    return {
        ...
        addresses: Array.from({ length: count }, (_, offset) => {
            ...
            return {
                index,
                address: branch.deriveChild(index).address.toLowerCase(),
            };
        }),
    };
}
...
export function deriveDepositAddress(mnemonic: string, index: number): string {
    return HDNodeWallet.fromMnemonic(Mnemonic.fromPhrase(mnemonic), 'm').derivePath(`m/44'/60'/0'/0/${index}`).address.toLowerCase();
}
