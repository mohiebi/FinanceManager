...
beforeEach(function () {
    ...
    // is a gitignored build artifact and CI only builds the client half, so on a
    // fresh checkout the gateway returns before dispatching: the guest test fails,
    // and the authenticated one passes for a reason that has nothing to do with
    // the rule it is meant to be pinning.
